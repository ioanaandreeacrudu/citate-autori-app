...
export default function PasswordGate({ children }) {
  // Verificăm sessionStorage la montare
  ...
  // Dacă este autentificat, redăm direct conținutul protejat (ManagePage)
    ...
    <div className="min-h-screen bg-indigo-50 flex items-center justify-center px-4">
      <div className={`bg-white rounded-2xl shadow-lg p-8 w-full max-w-sm border border-gray-100 ${shake ? "animate-bounce" : ""}`}>
        ...
        <p className="text-center mt-4">
          <a href="/" className="text-xs text-gray-400 hover:text-indigo-500">- Înapoi la citate</a>
        </p>
      </div>
    </div>
  );
