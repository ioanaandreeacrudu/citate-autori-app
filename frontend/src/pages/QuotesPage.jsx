    ...
    <div className="min-h-screen bg-indigo-50">
      <header className="sticky top-0 z-10 bg-white shadow-sm">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-indigo-600">Famous Quotes</h1>
        ...
        <div className="max-w-6xl mx-auto px-4 pb-4">
          <div className="relative">
            ...
            />
            {inputValue && (
              <button onClick={() => setInputValue("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 text-lg leading-none">
                ✕
              </button>
            )}
          </div>
          {searchTerm && <p className="text-xs text-indigo-500 mt-1 pl-1">Rezultate pentru: <strong>"{searchTerm}"</strong></p>}
        </div>
      </header>
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        {!error && !loading && quotes.length === 0 && (
          <div className="text-center py-20">
            ...
            {searchTerm ? (
              <button onClick={() => setInputValue("")} className="text-indigo-500 underline hover:text-indigo-700 text-sm">Șterge filtrele</button>
            ) : (
              <Link to="/manage" className="text-indigo-500 underline hover:text-indigo-700 text-sm">Adaugă primul citat →</Link>
            )}
          </div>
        )}
        {loading && (
